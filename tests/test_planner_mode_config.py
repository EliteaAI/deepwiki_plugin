...
of ``"deepagents"`` / ``"cluster"`` / ``"auto"`` — matching the canonical
strings used by helm values & docker-compose.
"""
...
def test_planner_type_alias_takes_precedence_over_env(agent):
    ...
    with patch.dict(os.environ, {"DEEPWIKI_STRUCTURE_PLANNER": "deepagents"}):
