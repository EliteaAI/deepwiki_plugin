"""
Tests for UnifiedRetriever — Phase 5 of Unified Graph & Clustering.

Tests the LangChain-compatible retriever backed by UnifiedWikiDB that
replaces WikiRetrieverStack (EnsembleRetriever + FAISS + BM25).
"""
...
# Module under test
# ---------------------------------------------------------------------------
from plugin_implementation.unified_retriever import (
    DEFAULT_FTS_WEIGHT,
    DEFAULT_VEC_WEIGHT,
    FTS_POOL,
    MAX_EXPANDED_DOCS,
...
class TestSearchRepository:
    ...
    def test_no_expansion(self, retriever):
        ...
        # With expansion should return >= without
    ...
    def test_expansion_marks_expanded_docs(self, retriever):
        ...
        # Expanded docs should reference what they were expanded from
        for d in expanded:
...
class TestGraphExpansion:
    def test_expand_finds_neighbours(self, retriever):
        ...
        assert "auth::AuthService" in ids  # initial kept
        assert "auth::TokenManager" in ids  # outgoing edge
    ...
    def test_expansion_filters_non_architectural(self, retriever):
        ...
        expanded = retriever._expand_documents([initial_doc])
        ids = [d.metadata["node_id"] for d in expanded]
        assert "auth::login" not in ids  # method filtered out
    ...
    def test_expansion_respects_cluster_boundary(self, retriever):
        ...
        ids = [d.metadata["node_id"] for d in expanded]
        # TokenManager -> DatabasePool crosses clusters (0 -> 1), shouldn't appear
    ...
    def test_expansion_cap(self, retriever):
        ...
        The cap gates *new expanded* additions; initial docs are always kept.
        With a small initial set and real DB edges, expansion should stop
        before exceeding the cap.
        """
        from langchain_core.documents import Document

        # Use a real node that has edges so expansion is exercised
        initial_doc = Document(
            page_content="class AuthService",
            metadata={
                "node_id": "auth::AuthService",
        ...
        expanded = retriever._expand_documents([initial_doc])
        # With one initial doc, total should stay within cap
    ...
    def test_expansion_preserves_all_initial_docs(self, retriever):
        ...
        # Initial docs are always preserved even if > cap
...
class TestEdgeCases:
    def test_expansion_with_nonexistent_node(self, retriever):
        ...
        assert len(expanded) >= 1  # At least the initial doc
    ...
    def test_incoming_edge_expansion(self, retriever):
        ...
        # TokenManager has an incoming edge from AuthService
        doc = Document(
            page_content="class TokenManager",
            metadata={
                "node_id": "auth::TokenManager",
        ...
        expanded = retriever._expand_documents([doc])
        ids = [d.metadata["node_id"] for d in expanded]
        # AuthService calls TokenManager → incoming edge
