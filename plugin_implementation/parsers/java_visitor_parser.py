...
class JavaVisitorParser(BaseParser):
    ...
    def _define_capabilities(self) -> LanguageCapabilities:
        ...
        return LanguageCapabilities(
            ...
            supported_symbols={
                ...
                SymbolType.FIELD, SymbolType.VARIABLE, SymbolType.CONSTANT,
                SymbolType.ENUM, SymbolType.ANNOTATION, SymbolType.PARAMETER,
                SymbolType.NAMESPACE, SymbolType.FUNCTION, SymbolType.MODULE
            },
            supported_relationships={
                ...
                RelationshipType.REFERENCES, RelationshipType.CONTAINS
            },
    ...
    def _setup_tree_sitter(self) -> bool:
        ...
        try:
            ...
            logger.info("Tree-sitter Java parser initialized successfully")
            return True
        except Exception as e:
    ...
    def visit_node(self, node: Node) -> None:
        ...
        if node.type == "package_declaration":
            ...
            self._handle_super_interfaces(node)
        elif node.type == "superclass":
            self._handle_superclass(node)
        else:
    ...
    def _handle_method_parameters(self, formal_params_node: Node, method_qualified_name: str) -> None:
        ...
        for child in formal_params_node.children:
            if child.type == "formal_parameter":
                ...
                # Create reference relationship for parameter type (if not built-in)
    ...
    def _handle_constructor_declaration(self, node: Node) -> None:
        ...
        symbol = Symbol(
            ...
            symbol_type=SymbolType.METHOD,  # Treat constructors as special methods
            file_path=self._current_file,
            range=self._create_range(node),
            scope=Scope.FUNCTION,
            parent_symbol=parent_symbol,
                ...
                for child in node.children:
                    if child.type in ["type_identifier", "generic_type", "array_type"]:
                        if child.type == "generic_type":
                            generic_params = self._extract_generic_type_parameters(child)
                            for param_type in generic_params:
                                # For generic parameters, determine if they represent composition
                                # Collections with entity types (Role, Permission, etc.) are composition
                                param_relationship_type = RelationshipType.COMPOSITION if not self._is_builtin_type(param_type) else RelationshipType.REFERENCES
    ...
    def _handle_object_creation(self, node: Node) -> None:
        ...
        for child in node.children:
            if child.type in ["type_identifier", "identifier"]:
                ...
                if type_name:
                    # Create constructor call relationship
                    source = self._get_current_scope_path() or self._current_file
                    
                    # Create a specific constructor call relationship
                    constructor_target = f"{type_name}.<init>"
                    
                    relationship = Relationship(
                        source_symbol=source,
                        target_symbol=constructor_target,
                        relationship_type=RelationshipType.CALLS,
                        source_file=self._current_file,
                    ...
                    # Also create a reference relationship to the class type
                    if not self._is_builtin_type(type_name):
                        ...
                        self._relationships.append(ref_relationship)
                break
            ...
            if child.type == "superclass":
                ...
                for grandchild in child.children:
                    if grandchild.type in ["type_identifier", "identifier"]:
                        ...
                        if parent_class:
                            relationship = Relationship(
                                source_symbol=class_symbol.name,
    ...
    def _handle_record_declaration(self, node: Node) -> None:
        ...
        # Create record symbol (treat as class for now)
    ...
    def parse_multiple_files(self, file_paths: List[str], max_workers: int = 4) -> Dict[str, ParseResult]:
        ...
        logger.info(f"Parsing {len(file_paths)} Java files for cross-file analysis with {max_workers} workers")
    ...
    def _parse_files_parallel(self, file_paths: List[str], max_workers: int) -> Dict[str, ParseResult]:
        """
        Parse multiple Java files in parallel using ProcessPoolExecutor.
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
            # Submit all tasks
            future_to_file = {executor.submit(_parse_single_java_file, file_path): file_path 
            ...
            for future in concurrent.futures.as_completed(future_to_file):
                ...
                if completed % 10 == 0:  # Log progress every 10 files
                    logger.info(f"Parsed {completed}/{len(file_paths)} Java files...")
    ...
    def _extract_generic_type_parameters(self, node: Node) -> List[str]:
        """Extract generic type parameters from a generic type node."""
        parameters = []
        ...
        for child in node.children:
            if child.type == "type_arguments":
                ...
                for type_arg_child in child.children:
                    if type_arg_child.type in ["type_identifier", "generic_type", "wildcard"]:
                        param_type = self._extract_type_name(type_arg_child)
                        if param_type and not self._is_builtin_type(param_type):
                            parameters.append(param_type)
                    elif type_arg_child.type == "wildcard":
                        # Handle wildcards like ? extends MyClass
                        for wildcard_child in type_arg_child.children:
                            if wildcard_child.type in ["type_identifier", "generic_type"]:
                                param_type = self._extract_type_name(wildcard_child)
                                if param_type and not self._is_builtin_type(param_type):
                                    parameters.append(param_type)
                break
        return parameters
    ...
    def _extract_node_source(self, node: Node) -> Optional[str]:
        ...
        try:
            ...
            logger.warning(f"Failed to extract source for node {node.type}: {e}")
            return None

# Register the parser
parser_registry.register_parser(JavaVisitorParser())
