...
if SEPARATE_DOC_INDEX:
    logger.info("[FEATURE FLAG] DEEPWIKI_DOC_SEPARATE_INDEX=1: Docs routed to vector store only (not graph)")
else:
    logger.debug("[FEATURE FLAG] DEEPWIKI_DOC_SEPARATE_INDEX=0: Docs included in graph (legacy mode)")
...
class EnhancedUnifiedGraphBuilder:
    ...
    def analyze_repository(self, repo_path: str, 
        ...
        with resource_monitor("graph_builder.generate_documents", logger):
            if stream_documents:
                if os.getenv("DEEPWIKI_KEEP_PARSE_RESULTS") != "1":
        ...
        keep_analysis = os.getenv("DEEPWIKI_KEEP_LAST_ANALYSIS") == "1"
        if keep_analysis:
        ...
        # Set DEEPWIKI_KEEP_CODE_SPLITTER_CACHE=1 to keep for debugging
        if os.getenv("DEEPWIKI_KEEP_CODE_SPLITTER_CACHE") != "1":
            try:
    ...
    def _build_multi_tier_graph(self, parse_results: Dict[str, Union[ParseResult, BasicParseResult]]) -> nx.MultiDiGraph:
        ...
        When DEEPWIKI_DOC_SEPARATE_INDEX=1, documentation files are excluded from the graph
