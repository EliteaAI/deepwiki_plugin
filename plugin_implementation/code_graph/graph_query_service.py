...
class GraphQueryService:
    ...
    across the entire DeepWiki pipeline.  Consumers (structure planner,
    ...
    def search(
        ...
        if self.fts and self.fts.is_open:
            return self._search_fts5(
                query, k=k, symbol_types=symbol_types,
                exclude_types=exclude_types, layer=layer,
    ...
    def _search_fts5(
        ...
        for doc in docs:
            ...
            if node_id and node_id in self.graph:
                connections = (self.graph.in_degree(node_id) +
    ...
    def _execute_index_clauses(self, jql) -> List[SymbolResult]:
        ...
        from .jql_parser import JQLQuery
        ...
        if name_value and not text_value:
            return self._jql_name_search(
                name_value, fetch_k,
                symbol_types=symbol_types, layer=layer,
                path_prefix=path_prefix,
            )
        ...
        if layer and self.fts and self.fts.is_open:
            return self._jql_layer_search(
                layer, fetch_k,
                symbol_types=symbol_types, path_prefix=path_prefix,
            )
    ...
    def _jql_name_search(
        self, name: str, k: int,
        symbol_types=None, layer=None, path_prefix=None,
    ) -> List[SymbolResult]:
        ...
        if self.fts and self.fts.is_open:
            try:
                ...
                rows = self.fts.search_by_name(
                    ...
                    exact=exact, k=k * 2, symbol_types=type_list,
                )
                results = []
                for doc in rows:
        ...
        return self._jql_full_scan(
            k, symbol_types=symbol_types,
            name_filter=name, layer=layer, path_prefix=path_prefix,
        )

    def _jql_layer_search(
        self, layer: str, k: int,
        symbol_types=None, path_prefix=None,
    ) -> List[SymbolResult]:
    ...
    def _jql_path_search(
        ...
        try:
            ...
            rows = self.fts.search_by_path_prefix(
                path_prefix, symbol_types=type_list, exclude_types=excluded, k=k * 2,
            )
            results = []
            for row in rows:
                meta = row if isinstance(row, dict) else getattr(row, 'metadata', row)
    ...
    def _jql_type_search(
        self, symbol_types: FrozenSet[str], k: int,
    ) -> List[SymbolResult]:
    ...
    def _jql_full_scan(
        self, k: int,
        symbol_types=None, layer=None, path_prefix=None, name_filter=None,
    ) -> List[SymbolResult]:
    ...
    def _apply_post_filters(
        self, results: List[SymbolResult], jql,
    ) -> List[SymbolResult]:
        ...
        if conn_clause:
            results = [
                r for r in results
                if conn_clause.matches_numeric(r.connections)
            ]
    ...
    def _filter_has_relationship(
        self, results: List[SymbolResult], edge_types: List[str],
    ) -> List[SymbolResult]:
