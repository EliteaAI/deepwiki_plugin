...
Usage::
    ...
    index = GraphTextIndex(cache_dir="~/.elitea/wiki_indexes")
...
from ..constants import DOC_SYMBOL_TYPES, classify_symbol_layer
...
class GraphTextIndex:
    ...
    ``~/.elitea/wiki_indexes``).  The file name is ``{cache_key}.fts5.db``
    ...
    def __init__(self, cache_dir: Optional[str] = None):
        if cache_dir is None:
            cache_dir = os.path.expanduser("~/.elitea/wiki_indexes")
    ...
    @property
    def _resolved_db_path(self) -> Optional[Path]:
    ...
    def build_from_graph(self, graph: nx.DiGraph, cache_key: str) -> int:
        ...
        try:
            ...
            logger.info(
                f"Built FTS5 index: {len(rows)} nodes indexed → {db_path.name}"
            )
            return len(rows)
    ...
    def search(
        ...
        sql = f"""\
            SELECT s.*, bm25(symbols_fts, {weights}) AS score
            FROM symbols_fts f
            JOIN symbols s ON s.id = f.rowid
            WHERE symbols_fts MATCH ?{where_clause}
            ORDER BY score
            LIMIT ?
        """
        ...
        try:
            ...
            if docs:
                ...
                logger.debug(
                    f"[FTS5] search({query!r:.50}, k={k}) → {len(docs)} results, "
                    f"top={top_names}"
                )
            return docs
    ...
    def search_symbols(
        ...
        *,
        ...
        if path_prefix and docs:
            ...
            docs = [
                d for d in docs
                if (d.metadata.get('rel_path') or '').startswith(prefix)
            ]
        ...
        *,
        ...
        try:
            ...
            try:
                ...
                # FTS5 query syntax error — fall back to simpler query
                logger.debug(
                    f"FTS5 query error for '{fts_query}': {exc}, "
                    f"falling back to simple query"
                )
            ...
            if docs:
                top_names = [d.metadata.get('symbol_name', '?') for d in docs[:5]]
                logger.debug(
                    f"[FTS5] search({original_query!r:.50}, k={k}) → "
                    f"{len(docs)} results, top={top_names}"
                )
            return docs
        except (sqlite3.InterfaceError, sqlite3.OperationalError, FileNotFoundError) as exc:
    ...
    def search_by_path_prefix(
        ...
        params.append(k)
        sql = f"SELECT * FROM symbols WHERE {where_clause} LIMIT ?"
        ...
        try:
            ...
            rows = conn.execute(sql, params).fetchall()
            logger.debug(
                f"[FTS5] search_by_path_prefix({prefix!r}) → {len(rows)} rows"
            )
    ...
    def get_by_node_id(self, node_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            conn = self._open_read_conn()
            row = conn.execute(
                "SELECT * FROM symbols WHERE node_id = ?", [node_id]
            ).fetchone()
            if row is None:
                return None
            return dict(row)
    ...
    @staticmethod
    def _row_to_document(row: sqlite3.Row, score: Optional[float] = None) -> Document:
