...
from .structure_skeleton import build_skeleton, StructureSkeleton
...
class WikiStructurePlannerEngine:
    ...
    def __init__(
        ...
        config: Optional[StructurePlannerConfig] = None,
    ...
    def _create_agent(self, collector):
        ...
        try:
            ...
            raise RuntimeError(f"DeepAgents not available: {e}")
    ...
    def plan_structure(
        ...
        try:
            ...
            task_prompt = get_structure_task_prompt(
                page_budget=self.config.page_budget,
                repo_name=repo_name,
                doc_clusters=self.config.doc_clusters
            )
            ...
            if len(self.tool_calls) == 0 and (stats['sections'] > 0 or stats['pages'] > 0):
                logger.info("[STRUCTURE_PLANNER] Deriving tool calls from collector (stream events didn't capture them)")
                ...
                for page_data in collector.pages:
                    self.tool_calls.append({
                        "tool": "define_page",
                        "args": {"page_name": page_data.get("page_name"), "section_name": page_data.get("section_name")},
                        "timestamp": datetime.now().isoformat(),
                        "derived": True
                    })
    ...
    def _validate_structure(self, parsed: Dict[str, Any], collector) -> None:
        ...
        if warnings:
            ...
            logger.info(
                f"[STRUCTURE_PLANNER][VALIDATION] Structure looks good: "
                f"{len(sections)} sections, {total_pages} pages"
            )
            ...
            if hasattr(msg, "tool_calls") and msg.tool_calls:
                for tc in msg.tool_calls:
                    ...
                    already_tracked = any(
                        ...
                        for t in self.tool_calls[-5:] if isinstance(t, dict)  # Check last 5
                    )
    ...
    def plan_structure_graph_first(
        ...
        try:
            ...
            logger.info(
                "[GRAPH_FIRST] Phase 1 – building skeleton "
                "(budget=%d, depth=%d, files=%d)",
            ...
            logger.info(
                "[GRAPH_FIRST] Phase 1 done in %.2fs: "
                "%d code clusters, %d doc clusters, %d/%d dirs",
                phase1_time,
            ...
            logger.info(
                "[GRAPH_FIRST] Phase 2 done in %.2fs: "
                "%d sections, %d pages",
                phase2_time,
            ...
            logger.info(
                "[GRAPH_FIRST] Complete in %.2fs "
                "(skeleton=%.2fs, refine=%.2fs)",
    ...
    def _collect_repository_files(self) -> List[str]:
        ...
        for dirpath, dirnames, filenames in os.walk(self.repo_root):
            ...
            dirnames[:] = [
                d for d in dirnames
                if d not in COVERAGE_IGNORE_DIRS and not d.startswith('.')
            ]
    ...
    def _validate_graph_first(
        ...
        if skeleton.total_arch_symbols > 0:
            ...
            if sym_pct < 80:
                warnings.append(
                    f"LOW SYMBOL COVERAGE: {sym_pct:.0f}% "
                    f"({page_symbol_count}/{skeleton.total_arch_symbols})"
                )
        ...
        if skeleton.total_dirs_covered > 0:
            ...
            if dir_pct < 90:
                warnings.append(
                    f"LOW DIR COVERAGE: {dir_pct:.0f}% "
                    f"({dirs_in_pages}/{skeleton.total_dirs_covered})"
                )

        # 2. Budget compliance
        if total_pages > self.config.page_budget * 2:
            warnings.append(
                f"OVER BUDGET: {total_pages} pages vs "
                f"budget {self.config.page_budget}"
            )
        ...
        empty = [
            ...
            if not p.get('target_symbols') and not p.get('target_folders')
            and not p.get('target_docs')
        ]
        if empty:
            warnings.append(
                f"EMPTY PAGES: {len(empty)} pages have no retrieval data: "
                f"{', '.join(empty[:5])}"
            )
        ...
        if warnings:
            ...
            logger.info(
                "[GRAPH_FIRST][VALIDATION] OK: %d sections, %d pages, "
                "%d symbols",
                len(sections), total_pages, page_symbol_count,
            )
