...
def prioritize_symbols(
    ...
    if max_symbols is None:
        max_symbols = int(
            os.environ.get('DEEPWIKI_MAX_TARGET_SYMBOLS', _DEFAULT_MAX_TARGET_SYMBOLS)
        )
    ...
    if pruned > 0:
        top_scores = ', '.join(
            f"{s.name}({s.layer}:{s.connections})" for s in selected[:5]
        )
        logger.info(
            "[REFINER][PRIORITIZE] Cluster pruned %d → %d symbols "
            "(cap=%d). Top-5: %s",
            len(symbols), len(unique_names), max_symbols, top_scores,
        )
...
def summarize_cluster(cluster: DirCluster) -> str:
    ...
    for sym in top_syms:
        desc = (
            sym.docstring.split('\n')[0][:120]
            if sym.docstring
            else f"{sym.type} in {sym.rel_path}"
        )
...
def refine_with_llm(
    ...
    if len(clusters) <= _BATCH_SIZE:
        llm_items = _single_llm_refinement(
            skeleton, llm, repo_name, page_budget,
        )
    else:
...
def _batched_llm_refinement(
    ...
    for batch_idx in range(n_batches):
        ...
        items = _single_llm_refinement(
            batch_skeleton, llm, repo_name, page_budget,
            batch_info=f"Batch {batch_idx + 1}/{n_batches}",
        )
        all_items.extend(items)
        logger.info(
            "[REFINER] Batch %d/%d → %d items",
            batch_idx + 1, n_batches, len(items),
        )
...
def assemble_wiki_structure(
    ...
    for item in llm_items:
        ...
        page = {
            ...
            'rationale': (
                f"Graph cluster {cid}: {len(cluster.dirs)} dirs, "
                f"{cluster.total_symbols} symbols"
            ),
    ...
    for cluster in skeleton.code_clusters:
        ...
        logger.warning(
            ...
            cluster.cluster_id, cluster.total_symbols,
        )
        dir_leaf = (
            cluster.dirs[0].rsplit('/', 1)[-1]
            if cluster.dirs else 'misc'
        )
        page = {
    ...
    doc_cluster_by_idx = {
        i: dc for i, dc in enumerate(skeleton.doc_clusters)
    }
    ...
    if skeleton.doc_clusters:
        uncovered = [
            dc for i, dc in enumerate(skeleton.doc_clusters)
            if i not in covered_doc_indices
        ]
        if uncovered:
            logger.warning(
                "[REFINER] %d/%d doc clusters not covered by LLM "
                "— adding fallback pages",
                len(uncovered), len(skeleton.doc_clusters),
            )
    ...
    for order, (section_name, pages) in enumerate(
        sorted(sections_map.items()), start=1,
    ):
        ...
        sections.append({
            ...
            'rationale': f"Grouped by graph-first LLM refinement",
            'pages': pages,
        })
    ...
    total_target = sum(
        len(p.get('target_symbols', []))
        for s in sections for p in s['pages']
    )
    max_cap = int(os.environ.get('DEEPWIKI_MAX_TARGET_SYMBOLS', _DEFAULT_MAX_TARGET_SYMBOLS))
    logger.info(
        "[REFINER][ASSEMBLE] %d pages, %d/%d arch symbols selected as "
        "target_symbols (cap=%d/page). Remaining covered by target_folders.",
        total_pages, total_target, total_arch, max_cap,
    )
...
def _add_doc_pages(
    ...
    for dc in doc_clusters:
        ...
        section_name = (
            parts[0].replace('_', ' ').replace('-', ' ').title()
        ...
        sections_map[section_name].append({
            'page_name': page_name,
            'description': (
                f"Documentation files in {dc.dir_path} "
                f"({dc.file_count} files)"
            ),
...
def _format_doc_clusters_for_prompt(doc_clusters: List[DocCluster]) -> str:
    ...
    for i, dc in enumerate(doc_clusters):
        ...
        lines.append(
            f"- **doc_{i}**: `{dc.dir_path}/` — {dc.file_count} files "
            f"({', '.join(dc.doc_types)})"
        )
