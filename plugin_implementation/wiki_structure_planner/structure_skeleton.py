...
_EDGE_WEIGHT: Dict[str, int] = {
    ...
    'inheritance': 3, 'implementation': 3, 'composition': 3,
    'overrides': 3,
    # Medium coupling (2) – standard cross-dir dependencies
    'imports': 2, 'calls': 2, 'creates': 2, 'instantiates': 2,
    'uses_type': 2, 'references': 2,
    # Weak coupling (1) – looser relationships
    'aggregation': 1, 'reads': 1, 'writes': 1, 'captures': 1,
    'specializes': 1,
...
def build_dir_symbol_map(
    ...
    for node_id, data in code_graph.nodes(data=True):
        ...
        dir_symbols[dir_path].append(SymbolInfo(
            name=sym_name,
            type=sym_type,
            rel_path=rel_path,
            layer=layer,
...
def build_dir_interaction_graph(
    ...
    for src, tgt, data in code_graph.edges(data=True):
        ...
        if src_dir not in dir_symbols or tgt_dir not in dir_symbols:
            continue
        rel_type = (
            data.get('relationship_type') or data.get('type') or ''
        ).lower()
...
def cluster_directories(
    ...
    total_edge_weight = sum(
        d.get('weight', 1) for _, _, d in dir_graph.edges(data=True)
    )
    if total_edge_weight < len(dir_graph.nodes) * 0.3:
        # Too few edges – Louvain won't produce meaningful communities
        logger.info(
            "[SKELETON] Sparse edge graph (%d nodes, total_weight=%d) "
            "→ falling back to directory-tree clustering",
            len(dir_graph.nodes), total_edge_weight,
        )
    ...
    for resolution in (0.5, 0.7, 1.0, 1.3, 1.6, 2.0, 2.5, 3.0):
        try:
            communities = list(nx.community.louvain_communities(
                dir_graph, weight='weight', resolution=resolution, seed=42,
            ))
        except Exception:
            continue
...
def normalize_clusters(
    ...
    for smol in small:
        ...
        best_overlap = -1
        smol_parents = {
            '/'.join(d.split('/')[:-1]) if '/' in d else '<root>'
            for d in smol
        }
        for idx, existing in enumerate(normal):
            ...
            total_after = (
                sum(len(dir_symbols.get(d, [])) for d in existing)
                + sum(len(dir_symbols.get(d, [])) for d in smol)
            )
    ...
    for cid, dirs_set in enumerate(normal, start=1):
        ...
        # Primary languages
        ext_counts = Counter(
            s.rel_path.rsplit('.', 1)[-1].lower()
            for s in symbols if '.' in s.rel_path
        )
    ...
    logger.info(
        "[SKELETON] Normalized %d raw clusters → %d DirClusters "
        "(total symbols=%d)",
        len(raw_clusters), len(result),
...
def _split_large(
    ...
    n_groups = max(2, math.ceil(
        sum(len(dir_symbols.get(d, [])) for d in cluster_dirs) / max_symbols
    ))
...
def build_doc_clusters(
    ...
    for group_key, dir_list in sorted(group_map.items()):
        ...
        doc_types = sorted(set(
            os.path.splitext(f)[1].lower() for f in merged_files
        ))
        # Use the group key as the representative path
        doc_clusters.append(DocCluster(
            dir_path=group_key,
    ...
    logger.info(
        "[SKELETON] Found %d doc-only clusters (%d total doc files, "
        "grouped from %d raw doc dirs)",
        len(doc_clusters),
...
def build_skeleton(
    ...
    logger.info(
        ...
        repo_name or 'repo', page_budget, effective_depth,
    ...
    logger.info(
        "[SKELETON] 1b. dir_interaction_graph: %d nodes, %d edges, "
        "total_weight=%d",
        dir_graph.number_of_nodes(), dir_graph.number_of_edges(),
        sum(d.get('weight', 1) for _, _, d in dir_graph.edges(data=True)),
    )

    # 1c. Cluster directories
    raw_clusters = cluster_directories(
        dir_graph, page_budget, dir_symbols,
    )
    logger.info(
    ...
    logger.info(
        ...
        avg_per_page, dyn_min, dyn_max,
    )
    code_clusters = normalize_clusters(
        raw_clusters, dir_symbols,
        min_symbols=dyn_min, max_symbols=dyn_max,
        page_budget=page_budget,
    )

    # 1e. Documentation clusters
    doc_clusters = build_doc_clusters(
        repository_files, dir_symbols, effective_depth,
    )
    ...
    logger.info(
        "[SKELETON] Complete: %d code clusters, %d doc clusters, "
        "%d/%d dirs covered, %d arch symbols",
        len(code_clusters), len(doc_clusters),
        skeleton.total_dirs_covered, skeleton.total_dirs_in_repo,
        total_arch,
    )
    return skeleton
