...
Feature-flagged via DEEPWIKI_UNIFIED_DB=1 (default 0).
...
def _attach_score_norm(row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        return row
    try:
        row["score_norm"] = 1.0 / (1.0 + math.exp(r))
    except OverflowError:
        # math.exp overflows for very large positive r → score → 0;
        # for very large negative r the expression underflows cleanly to 1.
        row["score_norm"] = 0.0 if r > 0 else 1.0
    return row
...
class UnifiedWikiDB:
    ...
    def __init__(
        ...
        *,
        ...
        logger.info(
            ...
            self.db_path, self._vec_available, self.embedding_dim, readonly,
        )
    ...
    def _load_extensions(self) -> None:
        ...
        if not _SQLITE_VEC_AVAILABLE or not _EXTENSIONS_LOADABLE:
            logger.debug(
                ...
                _SQLITE_VEC_AVAILABLE, _EXTENSIONS_LOADABLE,
            )
            return
        try:
    ...
    def _create_schema(self) -> None:
        ...
        if self._vec_available:
            try:
                cur.executescript(
                    _SCHEMA_VEC_TEMPLATE.format(dim=self.embedding_dim)
                )
            except Exception as exc:
    ...
    def get_nodes_by_cluster(
        self, macro: int, micro: Optional[int] = None, limit: int = 1000
    ) -> List[Dict[str, Any]]:
    ...
    def populate_embeddings(
        ...
        rows = self.conn.execute(
            "SELECT node_id, source_text FROM repo_nodes "
        ...
        for i in range(0, len(rows), batch_size):
            ...
            pairs = list(zip(node_ids, vectors))
    ...
    def search_hybrid(
        ...
        fts_results = self.search_fts5(
            query, path_prefix=path_prefix, cluster_id=cluster_id, limit=fts_k
        )
        ...
        if embedding and self._vec_available:
            vec_results = self.search_vec(
                embedding, k=vec_k, path_prefix=path_prefix, cluster_id=cluster_id
            )
    ...
    def set_cluster(
        self, node_id: str, macro: int, micro: Optional[int] = None
    ) -> None:
    ...
    def set_clusters_batch(
        self, assignments: List[Tuple[str, int, Optional[int]]]
    ) -> None:
    ...
    def set_hub(
        self, node_id: str, is_hub: bool = True, assignment: Optional[str] = None
    ) -> None:
    ...
    def get_cluster_nodes(
        self, macro: int, micro: Optional[int] = None
    ) -> List[str]:
    ...
    def get_all_clusters(self) -> Dict[int, Dict[int, List[str]]]:
        ...
        rows = self.conn.execute(
            ...
            "FROM repo_nodes WHERE macro_cluster IS NOT NULL"
        ).fetchall()
    ...
    def _nx_edge_to_dict(
        self, u: str, v: str, key: int, data: Dict
    ) -> Dict[str, Any]:
    ...
    def stats(self) -> Dict[str, Any]:
        ...
        langs = {}
        for row in self.conn.execute(
            "SELECT language, count(*) FROM repo_nodes GROUP BY language"
        ):
            langs[row[0]] = row[1]
        ...
        types = {}
        for row in self.conn.execute(
            "SELECT symbol_type, count(*) FROM repo_nodes GROUP BY symbol_type"
        ):
            types[row[0]] = row[1]
        ...
        edge_types = {}
        for row in self.conn.execute(
            "SELECT rel_type, count(*) FROM repo_edges GROUP BY rel_type"
        ):
            edge_types[row[0]] = row[1]

        # Cluster counts
        macro_count = self.conn.execute(
            "SELECT count(DISTINCT macro_cluster) FROM repo_nodes "
            "WHERE macro_cluster IS NOT NULL"
        ).fetchone()[0]

        hub_count = self.conn.execute(
            "SELECT count(*) FROM repo_nodes WHERE is_hub = 1"
        ).fetchone()[0]
