...
Note: File reading is handled by FilesystemMiddleware's read_file tool,
which reads actual files from the cloned repository.
"""
...
def _search_graph_by_text(code_graph: Any, query: str, k: int = 10) -> List[Document]:
    ...
    documents = []
    for score, node_id, node_data in scored[:k]:
...
# ---------------------------------------------------------------------------
# Unified-DB-backed helpers (used when DEEPWIKI_UNIFIED_RETRIEVER=1 and
...
def _get_unified_db_relationships(db_path: str, symbol_name: str,
    ...
    try:
        ...
        rows = conn.execute(
            "SELECT node_id, symbol_name, symbol_type FROM repo_nodes "
            "WHERE LOWER(symbol_name) = LOWER(?) LIMIT 5",
            (symbol_name,),
        ).fetchall()
        if not rows:
            ...
            rows = conn.execute(
                "SELECT node_id, symbol_name, symbol_type FROM repo_nodes "
                "WHERE LOWER(symbol_name) LIKE ? LIMIT 5",
                (f'%{symbol_name.lower()}%',),
            ).fetchall()
        if not rows:
...
def _find_graph_node(code_graph: Any, symbol_name: str, rel_path: str = '') -> Optional[str]:
        ...
        try:
            ...
            if retriever_stack:
                try:
                    ...
                    # Apply EmbeddingsFilter for semantic reranking if available
                    if EMBEDDINGS_FILTER_AVAILABLE and EmbeddingsFilter and embeddings:
                        try:
                            ...
                            vs_docs = list(embeddings_filter.compress_documents(
                                documents=vs_docs,
                                query=query
                            ))
                    ...
                    logger.info(
                        f"[SEARCH_CODEBASE] VS returned {len(vs_docs)} docs "
            ...
            try:
                if graph_text_index is not None and graph_text_index.is_open:
                    ...
                    logger.info(
                        f"[SEARCH_CODEBASE][UNIFIED_DB] {len(graph_docs)} results "
                        f"(query={query!r:.60})"
                    )
            except Exception as e:
            ...
            if HYBRID_FUSION_ENABLED and (all_docs or graph_docs):
                ...
                logger.info(
                    f"[SEARCH_CODEBASE][RRF] Fused {len(all_docs)} results "
                    f"from {list(ranked_lists.keys())}"
                )
            else:
                ...
                if graph_docs:
                    vs_symbols = {
                        ...
                        for doc in all_docs if doc.metadata.get('symbol_name')
                    }
                    for doc in graph_docs:
                ...
                all_docs.sort(key=lambda d: (
                    0 if d.metadata.get('search_source') == 'vectorstore' else 1
                ))
...
"""
        ...
        try:
            ...
            if query_service:
                ...
                if len(matching_nodes) > 1:
                    lines.append(f"**Other Matching Nodes:** {', '.join(f'`{n}`' for n in matching_nodes[:10] if n != target_node)}\n")
                
                # Group by depth and direction
                outgoing = [r for r in rels if r.source_name != symbol_name or r.hop_distance > 0]
                incoming = [r for r in rels if r.target_name == target_node or
                           (r.source_name != target_node and
                            any(r.target_name == target_node for _ in [1]))]
                    ...
                    for neighbor in code_graph.neighbors(node):
                        if neighbor not in visited:
                            ...
                            neighbors_by_depth[depth].append({
                                'node': neighbor,
                                'from': node,
                                'edge_data': code_graph.get_edge_data(node, neighbor)
                            })
            ...
            for depth, neighbors in neighbors_by_depth.items():
                ...
                for rel in neighbors:
                    edge_info = ""
                    if rel['edge_data']:
                        for key, data in rel['edge_data'].items():
                            if isinstance(data, dict):
        ...
        if code_graph is None:
            ...
            if _use_db_fallback:
                ...
                if db_docs:
                    ...
                    emit({"type": "tool_end", "tool": "search_graph",
                          "result_count": len(sections),
                          "timestamp": datetime.now().isoformat()})
                    return f"## Graph Search: {query}\n\nFound {len(sections)} symbols (unified DB):\n\n" + "\n\n---\n\n".join(sections)
    ...
    # Gated by DEEPWIKI_PROGRESSIVE_TOOLS=1 (off by default).
    ...
    def search_symbols(query: str, k: int = 20, symbol_type: str = '', file_prefix: str = '') -> str:
        ...
        try:
            ...
            for i, r in enumerate(results):
                ...
                lines.append(
                    f'{i+1}. `{r["name"]}` ({r["kind"]}{layer_str}) '
                    f'in {r["file"]} [{r["refs"]} refs]{doc_str}'
                )
            return '\n'.join(lines)
    ...
    def _orphan_fts_fallback(
        ...
        for succ in graph.successors(node_id):
            ...
            edges = edge_data.values() if isinstance(edge_data, dict) and all(
                isinstance(v, dict) for v in edge_data.values()
            ) else [edge_data]
            is_defines = any(
                e.get('relationship_type', '').lower() == 'defines'
                for e in edges
            )
            if not is_defines:
                continue
        ...
        for h in hits:
            lines.append(
                f'  \u26a1 `{h["method"]}` referenced in '
                f'`{h["ref_name"]}` ({h["ref_type"]}) \u2014 {h["ref_file"]}'
            )
        lines.append(
    ...
    def get_relationships_tool(symbol_name: str, direction: str = 'both', max_depth: int = 2) -> str:
        ...
        try:
            node_id, rels = query_service.resolve_and_traverse(
                symbol_name, direction=direction,
                max_depth=max_depth, max_results=50,
            )
            ...
            if not rels:
                ...
                orphan_hits = _orphan_fts_fallback(
                    node_id, code_graph, graph_text_index, emit,
                )
                if orphan_hits:
                    return orphan_hits
            ...
            for r in rels:
                ...
                is_outgoing = (
                    r.source_name == node_id
                    or src_low == symbol_name.lower()
                    or symbol_name.lower() in src_low
                )
                if is_outgoing:
    ...
    def search_docs(query: str, k: int = 5) -> str:
        ...
        try:
            ...
            if retriever_stack:
                try:
                    initial_k = min(k * 3, 15)
                    vs_docs = retriever_stack.search_repository(
                        query=query, k=initial_k, apply_expansion=False
                    )
                    if EMBEDDINGS_FILTER_AVAILABLE and EmbeddingsFilter and embeddings:
                        try:
                            ef = EmbeddingsFilter(
    ...
    # ================================================================
    _progressive = os.environ.get('DEEPWIKI_PROGRESSIVE_TOOLS', '').strip()
