...
def normalize_wiki_id(repo_identifier: str) -> str:
    ...
    Format: {owner}--{repo}--{branch} (all lowercase, special chars → dash)
...
class WikiRegistryManager:
    ...
    def register_wiki(
        ...
        # Find existing entry
        existing_idx = next(
            (i for i, w in enumerate(registry["wikis"]) if w["id"] == wiki_id),
            None
        )
    ...
    def delete_wiki_with_artifacts(self, wiki_id: str) -> Dict:
        ...
        try:
            ...
            wiki_artifacts = [
                a for a in (all_artifacts or [])
                if isinstance(a, dict) and a.get("name", "").startswith(wiki_prefix)
            ] if all_artifacts else []
            
            # Also handle direct name comparison if list returns different format
            if not wiki_artifacts and all_artifacts:
                wiki_artifacts = [
                    a for a in all_artifacts
                    if isinstance(a, str) and a.startswith(wiki_prefix)
                ]
    ...
    def get_wiki(self, wiki_id: str) -> Optional[Dict]:
        """
        ...
        return next(
            (w for w in registry["wikis"] if w["id"] == wiki_id),
            None
        )
    
    def list_wikis(self, filter_text: Optional[str] = None) -> List[Dict]:
        """
    ...
    def find_wiki_by_repo(self, repo: str, branch: Optional[str] = None) -> Optional[Dict]:
        """
