...
class FilterManager:
    ...
    def __init__(self, 
        ...
        logger.info(f"Filter manager initialized with {len(self.excluded_dirs)} excluded dirs, "
    ...
    def _load_repo_config(self, config_path: Optional[str]) -> Dict[str, Any]:
    ...
    def _build_excluded_dirs(self) -> Set[str]:
        ...
        defaults = {
            ...
            '.output', 'coverage', 'htmlcov', '.nyc_output'
        }
    ...
    def _build_allowed_extensions(self) -> Optional[Set[str]]:
        ...
        # Check repo config
        if ('file_filters' in self.repo_config and 
            'allowed_extensions' in self.repo_config['file_filters']):
            extensions = set()
        ...
        return {
            # Code files
            '.py', '.js', '.ts', '.jsx', '.tsx', '.java', '.go', '.cs', '.cpp', '.c',
            '.rb', '.php', '.rs', '.kt', '.scala', '.swift', '.dart', '.r', '.m',
    ...
    def _get_max_file_size(self) -> float:
        ...
        # Check repo config
        if ('repository' in self.repo_config and 
            'max_size_mb' in self.repo_config['repository']):
    ...
    def should_process_directory(self, dir_path: str) -> bool:
        ...
        for excluded_pattern in self.excluded_dirs:
            if (normalized_path == excluded_pattern or
                normalized_path.startswith(excluded_pattern + '/') or
                fnmatch.fnmatch(normalized_path, excluded_pattern)):
    ...
    def should_process_file(self, file_path: str, file_size_bytes: Optional[int] = None) -> bool:
        ...
        if file_size_bytes is not None:
            ...
            if file_size_mb > self.max_file_size_mb:
                logger.debug(f"File {file_path} excluded by size limit ({file_size_mb:.2f}MB > {self.max_file_size_mb}MB)")
                return False
        
        # Check excluded file patterns
        for excluded_pattern in self.excluded_files:
            if (file_name == excluded_pattern or
                fnmatch.fnmatch(file_name, excluded_pattern) or
                fnmatch.fnmatch(file_path, excluded_pattern)):
