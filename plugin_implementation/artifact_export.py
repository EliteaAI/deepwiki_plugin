...
# This wrapper preserves the owner/repo/branch signature for callers that
# don't have a combined repo_identifier string.
def normalize_wiki_id(owner: str = None, repo: str = None, branch: str = None,
                      repository: str = None) -> str:
    """
    Normalize repository identifier to wiki_id format.
    
    Delegates to registry_manager.normalize_wiki_id to ensure consistent
    normalization across the entire pipeline.
...
class ArtifactExporter:
    ...
    def export_wiki_artifacts(self, 
                             ...
                             export_formats: Optional[List[str]] = None) -> list:
        """
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                for format_name in export_formats:
                    try:
                        if format_name == "json":
                            jsons_artifacts = self._export_json_format(
    ...
    def _export_json_format(self,
        ...
        result_wiki_structure.append({
            ...
            'data': json.dumps(wiki_data, indent=4)
        })
