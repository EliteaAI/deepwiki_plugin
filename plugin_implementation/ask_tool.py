...
from .token_counter import get_token_counter, CONTEXT_TOKEN_BUDGET
...
ASK_USE_OPTIMIZED_CONTEXT = os.getenv("DEEPWIKI_ASK_USE_OPTIMIZED_CONTEXT", "1") == "1"
...
class AskTool:
    ...
    def __init__(
        ...
        thinking_callback: Optional[Callable[[Dict[str, Any]], None]] = None,
    ...
    def _optimize_query(self, question: str) -> str:
        ...
        self._emit_thinking(
            "query_optimization",
            "Optimizing query using repository context..."
        )
        ...
        user_prompt = QUERY_OPTIMIZATION_USER_PROMPT.format(
        ...
        try:
            ...
            response = llm_for_query.invoke([
                SystemMessage(content=QUERY_OPTIMIZATION_SYSTEM_PROMPT),
            ...
            if (optimized 
                ...
                and not ":" in optimized[:50]):  # Avoid responses like "Optimized query: ..."
    ...
    def _generate_answer(
        ...
        self._emit_thinking(
            "generating",
            f"Generating answer from {len(sources)} sources ({context_tokens:,} tokens)..."
        )
...
"""
        ...
        try:
            ...
            response = llm_for_answer.invoke([
