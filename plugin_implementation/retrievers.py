...
class SentenceTransformersCrossEncoderAdapter(BaseCrossEncoder if BaseCrossEncoder else object):
    ...
    def __init__(self, model_name: str, cache_folder: Optional[str] = None, device: str = 'cpu'):
...
class WikiRetrieverStack:
    ...
    def _initialize_retrievers(self):
        ...
        try:
            ...
            # Set DEEPWIKI_ENABLE_RERANKER=1 to enable
            enable_reranker = os.getenv("DEEPWIKI_ENABLE_RERANKER") == "1"
            if enable_reranker and CROSS_ENCODER_AVAILABLE and CrossEncoderReranker is not None and BaseCrossEncoder is not None:
                try:
                    rerank_top_n = 25
                    env_top_n = os.getenv("DEEPWIKI_RERANK_TOP_N")
                    if env_top_n:
                        try:
                            rerank_top_n = int(env_top_n)
                        except ValueError:
                            logger.warning(f"Invalid DEEPWIKI_RERANK_TOP_N='{env_top_n}', using default 25")

                    ce_model = SentenceTransformersCrossEncoderAdapter(
                        model_name=self.cross_encoder_model,
                        cache_folder=str(self.vectorstore_manager.model_cache_dir),
                        device='cpu',
                    )
    ...
    def search_repository(self, query: str, k: int = 15, apply_expansion: bool = True) -> List[Document]:
        ...
        try:
            if hasattr(self.repo_retriever, 'search_kwargs'):
                self.repo_retriever.search_kwargs = {"k": k}
            elif hasattr(self.repo_retriever, 'base_retriever') and hasattr(self.repo_retriever.base_retriever, 'search_kwargs'):
        ...
        if expanded_result:
            initially_retrieved_count = sum(1 for doc in expanded_result 
                                           if doc.metadata.get('is_initially_retrieved', False))
            logger.info(f"[RETRIEVAL] After expansion: {len(expanded_result)} docs, "
    ...
    def search_docs_semantic(self, query: str, k: int = 10, similarity_threshold: float = 0.7) -> List[Document]:
        ...
        Used when DEEPWIKI_DOC_SEMANTIC_RETRIEVAL=1 for:
        ...
        try:
            ...
            logger.info(f"[SEMANTIC_DOC_RETRIEVAL] Found {len(doc_candidates)} doc candidates from {len(raw_candidates)} total")
            ...
            if USE_SEMANTIC_DOC_RETRIEVAL and EMBEDDINGS_FILTER_AVAILABLE and EmbeddingsFilter:
                try:
                    ...
                    if embeddings:
                        ...
                        logger.debug(f"[SEMANTIC_DOC_RETRIEVAL] Calling compress_documents with {len(doc_candidates)} docs")
                        reranked = embeddings_filter.compress_documents(
            ...
            logger.info(
                f"[SEMANTIC_DOC_RETRIEVAL] Returning {len(doc_candidates)} docs for query: '{query[:50]}...'"
            )
