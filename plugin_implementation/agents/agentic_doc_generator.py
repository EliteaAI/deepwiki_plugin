...
class DocumentationState:
    ...
    def get_cross_reference(self, symbol_id: str) -> Optional[str]:
            ...
            "function": {
                ...
                "parameters": {
                    ...
                    "properties": {
                        "content": {
                            "type": "string",
                            "description": "The markdown documentation for this section"
                        },
                        "summary": {
                            "type": "string",
                            "description": "One-sentence summary for the table of contents"
                        }
                    },
...
class AgenticDocGenerator:
    ...
    def __init__(
        ...
        if expanded_docs and len(expanded_docs) > 0:
            ...
            logger.debug(f"Sample metadata: is_initially_retrieved={sample_doc.metadata.get('is_initially_retrieved')}, "
        ...
        for doc in expanded_docs:
            ...
            is_anchor = (
                doc.metadata.get('is_initially_retrieved', False) or 
                doc.metadata.get('initially_retrieved', False)
            )
        ...
        logger.info(f"AgenticDocGenerator initialized: {len(self.anchor_docs)} anchors, "
                   f"{len(self.expanded_deps)} expanded deps")
        ...
        if not self.anchor_docs and expanded_docs:
            ...
            for i, doc in enumerate(expanded_docs[:5]):  # Sample first 5
                logger.warning(f"  Doc {i}: symbol={doc.metadata.get('symbol_name')}, "
        ...
        if SKIP_REPO_CONTEXT_FOR_PAGES:
            logger.info(f"⚡ SKIP_REPO_CONTEXT_FOR_PAGES=1: Omitting {len(repo_context or ''):,} char repository_context (agentic v1)")
    ...
    def _get_doc_id(self, doc) -> str:
        ...
        return (
            doc.metadata.get('node_id') or 
            doc.metadata.get('symbol_name') or 
    ...
    def _cluster_into_sections(self) -> List[SectionContext]:
        ...
        for cat_key, section_title, section_desc in section_templates:
            ...
            for doc in cat_docs:
                ...
                if section_tokens + doc_tokens > self.SECTION_TOKEN_BUDGET and section_docs:
                    ...
                    sections.append(SectionContext(
                        name=name,
                        anchor_ids=[self._get_doc_id(d) for d in section_docs]
                    ))
                    section_docs = []
                    section_tokens = 0
                    section_num += 1
            ...
            if i == largest_idx and len(section.anchor_ids) >= sections_needed * 2:
                ...
                for j in range(sections_needed):
                    ...
                    result.append(SectionContext(
                        name=part_name,
                        anchor_ids=section.anchor_ids[start:end]
                    ))
            else:
    ...
    def _extract_signature(self, doc) -> str:
        ...
        for i, line in enumerate(lines[:15]):
            ...
            if line_stripped.startswith(('def ', 'async def ', 'class ', 'function ', 'interface ', 'type ', 'struct ', 'enum ')):
                ...
                for j in range(i+1, min(i+5, len(lines))):
                    ...
                    if next_line.startswith(('"""', "'''", '//', '/*', '*')):
                        ...
                        docstring_preview = next_line.strip('"\'/\' *').strip()[:80]
    ...
    def _handle_tool_calls(self, response, section: SectionContext) -> Optional[str]:
        """
        ...
        for idx, tool_call in enumerate(tool_calls):
            ...
            args_preview = {k: (v[:100] + '...' if isinstance(v, str) and len(v) > 100 else v) 
                          for k, v in tool_args.items()} if tool_args else {}
            ...
            if tool_name == 'commit_section':
                ...
                if content:
                    ...
                    self.doc_state.add_section(
                        section.name,
                        section.anchor_ids,
                        summary
                    )
                    committed_this_turn = True
                    logger.info(f"    ✓ Committed section '{section.name}' ({len(content)} chars, summary: '{summary[:50]}...')")
                else:
                ...
                if requested_name:
                    ...
                    if requested_name in valid_sections:
                        ...
                        logger.warning(f"    ⚠️ Invalid section requested: '{requested_name}'. Available: {valid_sections}")
                else:
                    logger.info(f"    → Auto-advancing to next section")
    ...
    def generate(self, config: RunnableConfig) -> str:
        ...
        logger.info(f"🚀 Starting agentic generation for '{self.page_spec.page_name}' "
        ...
        while self.current_section_idx < len(self.sections) and iteration < max_iterations:
            ...
            logger.info(f"   📝 Context built: {len(section.full_code)} chars code, {len(section.signatures)} chars signatures")
        ...
        logger.info(f"✅ Agentic generation complete:")
    ...
    def _assemble_final_document(self) -> str:
        ...
        if len(self.committed_content) > 1:
            # Generate ToC
            toc_lines = ["## Contents\n"]
            for section_name, symbols, summary in self.doc_state.toc:
