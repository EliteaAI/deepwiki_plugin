...
from ..prompts.wiki_prompts_enhanced import ENHANCED_WIKI_STRUCTURE_PROMPT, TARGET_AUDIENCES, \
    ...
    QUALITY_ASSESSMENT_PROMPT, COMPRESSED_CONTENT_GENERATION_PROMPT_V3, ENHANCED_CONTENT_GENERATION_PROMPT_V2, \
    ENHANCED_CONTENT_GENERATION_PROMPT_OLD, ENHANCED_CONTENT_GENERATION_PROMPT_COMPRESSED, \
    ENHANCED_CONTENT_GENERATION_PROMPT_V3, ENHANCED_CONTENT_GENERATION_PROMPT_V3_REORDERED, \
    ENHANCED_CONTENT_GENERATION_PROMPT_V5, ENHANCED_CONTENT_GENERATION_PROMPT_V3_TONE_ADJUSTED, \
    ENHANCED_CONTENT_GENERATION_PROMPT_V3_WITH_CONTINUATION
from ..retrievers import WikiRetrieverStack
from ..constants import DOC_SYMBOL_TYPES, EXPANSION_SYMBOL_TYPES, ARCHITECTURAL_SYMBOLS, \
    DOCUMENTATION_EXTENSIONS_SET, KNOWN_FILENAMES, COVERAGE_IGNORE_DIRS, DOT_DIR_WHITELIST
from ..code_graph.expansion_engine import expand_smart, has_relationship, SMART_EXPANSION_ENABLED
from ..state.wiki_state import WikiStyle, TargetAudience, WikiState, WikiStructureSpec, PageGenerationState, WikiPage, \
    QualityAssessmentState, QualityAssessment, EnhancementState, PageSpec, RepositoryAnalysis, ConceptualRepositorySummary, \
    SectionSpec
from ..document_compressor import DocumentCompressor
from ..document_ranker import DocumentRanker
from ..token_counter import TokenCounter, CONTEXT_TOKEN_BUDGET, get_token_counter
...
SKIP_REPO_CONTEXT_FOR_PAGES = os.getenv("DEEPWIKI_SKIP_REPO_CONTEXT_FOR_PAGES", "0") == "1"
...
USE_STRUCTURED_REPO_ANALYSIS = os.getenv("DEEPWIKI_USE_STRUCTURED_REPO_ANALYSIS", "0") == "1"

# =============================================================================
# Feature Flags for Doc/Code Separation
# =============================================================================
# When enabled, documentation is retrieved from vector store (semantic) instead of graph
# Requires DEEPWIKI_DOC_SEPARATE_INDEX=1 during indexing to be effective
SEPARATE_DOC_INDEX = os.getenv("DEEPWIKI_DOC_SEPARATE_INDEX", "0") == "1"
...
USE_SEMANTIC_DOC_RETRIEVAL = os.getenv("DEEPWIKI_DOC_SEMANTIC_RETRIEVAL", "0") == "1"
...
AUTO_TARGET_DOCS = os.getenv("DEEPWIKI_AUTO_TARGET_DOCS", "0") == "1"
...
class OptimizedWikiGenerationAgent:
    ...
    def __init__(self,
                 ...
                 graph_text_index: Optional[Any] = None):
        ...
        self.branch = branch
        self.elitea_client = elitea_client
        ...
        # Cluster expansion state (Phase 6): lazily populated when cluster
        # mode is active so the unified DB is opened once and reused across
    ...
    def analyze_repository(self, state: WikiState, config: RunnableConfig) -> Dict[str, Any]:
        ...
        try:
            ...
            if not repository_files:
                repository_files = list(set(
                    doc.metadata.get('source', '') for doc in all_documents if doc.metadata.get('source')
                ))
    ...
    def generate_wiki_structure(self, state: WikiState, config: RunnableConfig) -> Dict[str, Any]:
        ...
        try:
            ...
            # consulted. Accepted values: "deepagents" (aliases:
            # "agent"/"agentic"), "cluster", "auto" (default — file-count
            # based).
                ...
                try:
                    ...
                    if this and getattr(this, 'module', None):
                        ...
                        this.module.invocation_thinking(
                            ...
                            f"Structure designed: {len(response.sections)} sections / "
                            f"{total_pages} pages (cluster-based)\n"
                            f"Reasoning: Mathematical clustering with cheap LLM naming.\n"
            ...
            if use_deepagents:
                ...
                try:
                    ...
                    logger.info(f"WikiStructureSpec created successfully (deepagents)")
                    logger.info(f"Wiki structure sections: {len(response.sections)}")
                    if this and getattr(this, 'module', None):
                        total_pages = sum(len(s.pages) for s in response.sections)
                        this.module.invocation_thinking(
            ...
            try:
                ...
                if os.getenv("DEEPWIKI_LOG_LLM_STRUCTURE_RAW") == "1":
                    raw = raw_text or ""
                    raw_len = len(raw)
                ...
                logger.warning(
                    f"WikiStructureSpec validation failed; using fallback structure. Error: {ve}")
    ...
    def dispatch_page_generation(self, state: WikiState, config: RunnableConfig) -> List[Send]:
        ...
        for section_idx, section in enumerate(wiki_structure.sections):
            for page_idx, page in enumerate(section.pages):
                ...
                page_spec_dict = page.model_dump() if hasattr(page, 'model_dump') else (page.dict() if hasattr(page, 'dict') else page.__dict__)
    ...
    def _split_overloaded_pages(self, wiki_structure) -> None:
        ...
        max_syms = self._get_env_int("DEEPWIKI_MAX_SYMBOLS_PER_PAGE", default_cap)
        total_splits = 0
        ...
        if total_splits > 0:
            wiki_structure.total_pages = sum(
                len(s.pages) for s in wiki_structure.sections
            )
            logger.info(
    ...
    def generate_page_content(self, state: PageGenerationState, config: RunnableConfig) -> Dict[str, Any]:
        ...
        try:
            ...
            if relevant_content.get("mode") == "agentic":
                ...
                generated_content = self._generate_agentic(
                    page_spec, relevant_content, repo_context, config
                )
                ...
                generated_content = self._generate_with_continuation(
                    page_spec, relevant_content, repo_context, config
                )
            else:
            ...
            try:
                from ..diagram_sanitizer import sanitize_content, SanitizerConfig
        ...
        try:
            ...
            response = await self.llm.ainvoke(
                enhancement_prompt.format_messages(
                    ...
                    validation_feedback=json.dumps(quality_assessment.model_dump() if hasattr(quality_assessment, 'model_dump') else (quality_assessment.dict() if hasattr(quality_assessment, 'dict') else {}), indent=2),
    ...
    def finalize_wiki(self, state: WikiState, config: RunnableConfig) -> Dict[str, Any]:
        ...
        try:
            ...
            # Close cluster DB if it was opened during page expansion
            self._close_cluster_db()
    ...
    def _build_graph(self) -> CompiledStateGraph:
        ...
        # Set DEEPWIKI_CHECKPOINT_BACKEND=memory to enable checkpointing if needed
        checkpoint_backend = os.getenv("DEEPWIKI_CHECKPOINT_BACKEND", "none").lower()
    ...
    def generate_wiki(self, user_message: str) -> Dict[str, Any]:
        ...
        try:
            ...
            wiki_structure_dict = (
                wiki_structure.model_dump() if wiki_structure and hasattr(wiki_structure, 'model_dump') else (
                    wiki_structure.dict() if wiki_structure and hasattr(wiki_structure, 'dict') else wiki_structure
                )
            )
            ...
            return {
                ...
                "generation_summary": final_state.get("generation_summary") or {},
    ...
    def _create_repository_analysis(self, file_paths: List[str], index_stats: Dict[str, Any],
                                    readme_content: str = "") -> RepositoryAnalysis:
        ...
        return RepositoryAnalysis(
            ...
            documentation_files=[p for p in unique_files if any(p.lower().endswith(ext) for ext in ['.md', '.rst', '.txt'])],
            configuration_files=[p for p in unique_files if any(p.lower().endswith(ext) for ext in ['.json', '.yaml', '.yml', '.toml', '.ini', '.cfg'])],
    ...
    def _resolve_planner_choice(self, config: Optional[RunnableConfig]) -> str:
        ...
        Returns one of ``"deepagents"`` / ``"cluster"`` / ``"auto"`` (lower-case).
        ``"agent"``, ``"agentic"`` and ``"deepagents"`` are aliases
        normalised to the canonical ``"deepagents"`` value (matching
        ``DEEPWIKI_STRUCTURE_PLANNER`` in helm values & docker-compose).
    ...
    def _should_use_deepagents_structure_planner(
        ...
        token_threshold = self._get_env_int("DEEPWIKI_DEEPAGENTS_REPOCTX_TOKENS", 8000)
    ...
    # ── Cluster-based structure planning (Phase 4) ────────────────

    def _generate_wiki_structure_with_clusters(
        self, config: Optional[RunnableConfig] = None,
    ) -> WikiStructureSpec:
        """Generate structure from pre-computed clusters via ClusterStructurePlanner.

        Requires:
        - ``DEEPWIKI_UNIFIED_DB=1`` (Phases 1-3 ran during indexing)
        - A ``.wiki.db`` file in the graph cache directory

        Raises RuntimeError if the DB cannot be found.
        """
        db_path = self._find_unified_db()
        if not db_path:
            raise RuntimeError(
                "Cluster planner requires a unified DB (.wiki.db) "
                "but none was found. Ensure DEEPWIKI_UNIFIED_DB=1 during indexing."
            )

        logger.info("Cluster planner: opening %s", db_path)
        with UnifiedWikiDB(db_path) as udb:
            # Verify Phase 3 actually ran
            if not udb.get_meta("phase3_completed"):
                raise RuntimeError(
                    "Cluster planner requires Phase 3 (clustering) to be "
                    "completed in the unified DB, but phase3_completed is not set."
                )
            ...
            # Cache the DB path so _try_cluster_expansion can reuse it
            # during page generation without re-searching.
            self._cluster_db_path = db_path
            return spec

    # ── Cluster-bounded page expansion (Phase 6) ─────────────────
    ...
    def _try_cluster_expansion(
        ...
        The unified DB is opened lazily on the first call and kept open
        for the duration of the wiki generation (closed in
        ``_close_cluster_db``).
        """
        # Only activate on pages produced by the cluster planner.
        ...
        if total_tokens <= CONTEXT_TOKEN_BUDGET:
            ...
            # Rare: expansion exceeded budget — apply ranked truncation
            logger.info(
    ...
    def _open_cluster_db(self) -> Optional[UnifiedWikiDB]:
    ...
    def _close_cluster_db(self) -> None:
        """Close the cached unified DB (call at end of wiki generation)."""
        if self._cluster_db is not None:
            try:
    ...
    @staticmethod
    def _extract_macro_id(rationale: str) -> Optional[int]:
        """Parse ``macro=N`` from a cluster-planner rationale string.

        Delegates to :func:`cluster_utils.extract_macro_id` so the regex
        logic is importable from lightweight tests without pulling in the
        full wiki agent stack.
        """
        from ..cluster_utils import extract_macro_id
        return extract_macro_id(rationale)

    def _find_unified_db(self) -> Optional[str]:
        """Locate the ``.wiki.db`` file produced by the indexer.

        Uses the same discovery strategy as FAISS / BM25 / graph indexes:
        1. Compute ``cache_key = md5(repo_path + commit_hash)`` directly
           from indexer state (fastest, works in-process).
        2. Look up ``unified_db`` section of ``cache_index.json``.
        3. Fall back to the most recent ``*.wiki.db`` in cache_dir.
        """
        import glob

        cache_dir = None
        if hasattr(self.indexer, 'graph_manager') and self.indexer.graph_manager:
            cache_dir = getattr(self.indexer.graph_manager, 'cache_dir', None)
        if not cache_dir:
            cache_dir = "/tmp/wiki_builder/cached_graphs"
        ...
        # 1. Direct hash computation (same key as FAISS / BM25 / docstore)
        repo_path = getattr(self.indexer, 'current_repo_path', None)
        commit_hash = getattr(self.indexer, '_last_commit_hash', None)
        if repo_path:
            vs_mgr = getattr(self.indexer, 'vectorstore_manager', None)
            if vs_mgr:
                cache_key = vs_mgr._generate_repo_hash(repo_path, [], commit_hash)
                db_path = os.path.join(cache_dir, f"{cache_key}.wiki.db")
                if os.path.exists(db_path):
                    return db_path

        # 2. cache_index.json lookup (works after download on controller)
        try:
            ...
            if isinstance(udb_map, dict):
                for _rid, cache_key in udb_map.items():
                    db_path = os.path.join(cache_dir, f"{cache_key}.wiki.db")
                    if os.path.exists(db_path):
                        return db_path
        except Exception:
        ...
        # 3. Glob fallback (most recent .wiki.db)
    ...
    def _detect_doc_clusters(self, dir_files: Dict[str, List[str]]) -> List[Dict[str, Any]]:
        ...
        CODE_EXTENSIONS = {'.py', '.js', '.ts', '.java', '.go', '.rs', '.cpp', '.cc', '.cxx', '.c++', '.c', '.h', '.hpp', '.hh', '.hxx', '.rb', '.php'}
    ...
    def _get_graph_top_nodes(self, graph: Optional[Any], limit: int = 20) -> List[Dict[str, Any]]:
    ...
    def _generate_wiki_structure_with_deepagents(
        ...
        try:
            ...
            raise RuntimeError(f"WikiStructurePlannerEngine unavailable: {e}")
        ...
        page_budget_env = self._get_env_int("DEEPWIKI_PAGE_BUDGET", 0)
        if page_budget_env > 0:
            ...
            min_pages = self._get_env_int("DEEPWIKI_PAGE_BUDGET_MIN", 8)
            default_max_pages = 200 if file_count >= 5000 or graph_nodes >= 100000 else 80
            max_pages = self._get_env_int("DEEPWIKI_PAGE_BUDGET_MAX", default_max_pages)
            files_per_page = self._get_env_int("DEEPWIKI_PAGE_FILES_PER_PAGE", 100)  # Fewer pages per file (broad pages)
            dirs_per_page = self._get_env_int("DEEPWIKI_PAGE_DIRS_PER_PAGE", 4)  # Fewer pages per dir (broad pages)
            graph_divisor = self._get_env_int("DEEPWIKI_PAGE_GRAPH_NODES_DIVISOR", 5)  # Lower graph influence (broad pages)
            ...
            logger.info(
                ...
                page_budget, file_count, dir_count, graph_nodes
            )
        ...
        max_iterations_cap = self._get_env_int("DEEPWIKI_DEEPAGENTS_MAX_ITERATIONS_CAP", default_iterations_cap)
        ...
        max_iterations_env = self._get_env_int("DEEPWIKI_DEEPAGENTS_MAX_ITERATIONS", 0)
        ...
        logger.info(
            ...
            max_iterations, page_budget, dir_count_for_iterations, calculated_iterations
        )
        ...
        planner_config = StructurePlannerConfig(
            ...
            max_ls_calls=self._get_env_int("DEEPWIKI_DEEPAGENTS_MAX_LS", max_ls_default),
            max_glob_calls=self._get_env_int("DEEPWIKI_DEEPAGENTS_MAX_GLOB", max_glob_default),
            max_read_calls=self._get_env_int("DEEPWIKI_DEEPAGENTS_MAX_READ", max_read_default),
            max_grep_calls=self._get_env_int("DEEPWIKI_DEEPAGENTS_MAX_GREP", max_grep_default),
        ...
        # Default-on for the DeepAgents planner — set DEEPWIKI_DEEPAGENTS_LOG_STREAM=0 to silence.
        ...
        data = engine.plan_structure(
            repo_name=repo_name,
            stream_callback=stream_callback if log_stream else None
        )
        ...
        if os.getenv("DEEPWIKI_DEEPAGENTS_COVERAGE_CHECK", "1") != "0":
    ...
    def _generate_wiki_structure_graph_first(
        ...
        Feature-flagged by ``DEEPWIKI_USE_GRAPH_FIRST=1``.
        """
        ...
        page_budget_cap = int(os.getenv("DEEPWIKI_PAGE_BUDGET_CAP", "200"))
        page_budget = max(8, min(raw_budget, page_budget_cap))

        effective_depth = detect_effective_depth(repository_files)

        logger.info(
            "[GRAPH_FIRST] repo=%s budget=%d depth=%d files=%d dirs=%d "
            "graph_nodes=%d",
            repo_name, page_budget, effective_depth, file_count,
            dir_count,
    ...
    def _apply_deepagents_coverage_check(
            ...
            for page in section.pages:
                ...
                for tf in (page.target_folders or []):
                    ...
                    for dir_path in all_expected:
                        ...
                        if (dp_lower == tf_norm or
                                dp_lower.startswith(tf_norm + '/') or
                                tf_norm.startswith(dp_lower + '/')):
                            covered_dirs.add(dir_path)

                # Secondary: check page_name / description for full dir paths
                hay = ' '.join(
                    p for p in [
                        page.page_name, page.description,
                        page.content_focus,
        ...
        for path in repository_files:
            ...
            if ext.lower() not in {
                ...
                '.cs', '.swift', '.kt', '.kts', '.scala', '.php',
                '.vue', '.svelte', '.lua', '.r', '.jl', '.ex', '.exs', '.erl', '.hs',
                '.clj', '.cljs', '.ml', '.fs', '.fsx', '.dart', '.groovy', '.gradle',
            }:
                continue
            parts = path.split("/")
            for depth in range(1, min(len(parts), max_depth + 1)):
        ...
        max_missing = self._get_env_int("DEEPWIKI_DEEPAGENTS_COVERAGE_MAX_DIRS", max_missing_default)
        ...
        page_budget_overflow = self._get_env_int("DEEPWIKI_PAGE_BUDGET_OVERFLOW", coverage_overflow_default)
        ...
        added = 0
        use_llm = os.getenv("DEEPWIKI_DEEPAGENTS_COVERAGE_LLM", "1") != "0"
    ...
    def _normalize_structure_payload(self, payload: Any) -> Dict[str, Any]:
        ...
        if normalized.get("overview") is None:
            normalized["overview"] = (
                ...
                or normalized.get("introduction")
                or ""
            )
        ...
        for s_idx, section in enumerate(sections_raw):
            ...
            for p_idx, page in enumerate(pages_raw):
                ...
                page_name = (
                    ...
                    or page.get("title")
                    or page.get("name")
                    or f"{section_name} Page {p_idx + 1}"
                )
    ...
    def _get_docs_by_target_symbols(self, target_symbols: List[str], max_hops: int = 1, target_folders: List[str] = None) -> List[Document]:
        """
        ...
        if unmatched_symbols:
            ...
            if fts is not None and fts.is_open:
                ...
                for target in unmatched_symbols:
                    ...
                    docs = fts.search_by_name(
                        target, exact=True, k=5,
                        symbol_types=ARCHITECTURAL_SYMBOLS,
                        language=page_lang,
                    )
                    if not docs:
                        docs = fts.search_by_name(
                            target, exact=False, k=5,
                            symbol_types=ARCHITECTURAL_SYMBOLS,
                            language=page_lang,
                        )
                    if docs:
                        ...
                        logger.debug(
                            f"[GRAPH_RETRIEVAL][FTS5] '{target}' → {len(docs)} hits"
                        )
                    else:
                        logger.debug(
                            f"[GRAPH_RETRIEVAL][FTS5] '{target}' → 0 hits"
                        )
            ...
            if still_unmatched:
                logger.debug(f"[GRAPH_RETRIEVAL] O(N) scan for {len(still_unmatched)} still-unmatched symbols: {still_unmatched}")
                for node_id, node_data in graph.nodes(data=True):
                    ...
                    for target in still_unmatched:
                        ...
                        if (symbol_name == target or 
                            symbol_name.endswith(f".{target}") or
                            node_id_symbol == target):
            ...
            if lang_counts:
                ...
                if len(lang_counts) > 1 and majority_count / total_with_lang >= 0.6:
                    ...
                    for sym in list(symbol_to_nodes):
                        symbol_to_nodes[sym] = [
                            nid for nid in symbol_to_nodes[sym]
        ...
        if not matched_nodes:
            ...
            for node_id in list(graph.nodes())[:10]:
                ...
                sample_node_ids.append(f"{node_id[:80]}... ({sym_name})" if len(str(node_id)) > 80 else f"{node_id} ({sym_name})")
        ...
        if not SMART_EXPANSION_ENABLED or not locals().get('SMART_EXPANSION_ENABLED_LOCAL', False):
            ...
            if max_hops > 0:
                for node_id in matched_nodes:
                    try:
                        predecessors = [n for n in graph.predecessors(node_id)
                                       if graph.nodes.get(n, {}).get('symbol_type') in EXPANSION_SYMBOL_TYPES]
                        successors = [n for n in graph.successors(node_id)
                                     if graph.nodes.get(n, {}).get('symbol_type') in EXPANSION_SYMBOL_TYPES]
                        ...
                        pass
            logger.info(
                f"[GRAPH_RETRIEVAL][NAIVE] Expanded to {len(expanded_nodes)} nodes "
                f"(from {len(matched_nodes)} initial)"
            )
        ...
        for node_id in expanded_nodes:
            ...
            doc = self._node_to_document(
                node_id, graph, augmented_content=augmented_content
            )
            if doc is None:
                continue
    ...
    def _get_docs_from_store_semantic(self, page_spec: 'PageSpec', max_docs: int = 10) -> List[Document]:
        ...
        This method is used when DEEPWIKI_DOC_SEPARATE_INDEX=1 (docs not in graph)
        or when DEEPWIKI_DOC_SEMANTIC_RETRIEVAL=1 (prefer semantic over path-based).
    ...
    def _get_doc_nodes_from_graph(
        ...
        When DEEPWIKI_DOC_SEPARATE_INDEX=1, docs are NOT in the graph (code-only).
        ...
        if SEPARATE_DOC_INDEX:
            logger.info(
                "[DOC_RETRIEVAL] SEPARATE_DOC_INDEX=1: Docs not in graph. "
                "Use semantic retrieval from vector store."
            )
            return []
        ...
        if fts is not None and fts.is_open:
            doc_candidates = self._get_doc_nodes_fts5(
                fts, target_docs, page_name, target_symbols,
                page_keywords, symbol_keywords, max_docs
            )
        else:
            # --- Brute-force fallback: O(N) graph scan ---
            doc_candidates = self._get_doc_nodes_brute(
                graph, target_docs, page_name, target_symbols,
                page_keywords, symbol_keywords, max_docs
            )
        ...
        if documents:
            ...
            logger.info(f"[DOC_RETRIEVAL] Retrieved {len(documents)} documentation nodes from graph "
    ...
    def _score_doc_candidate(
        ...
        if page_name:
            page_topic_keywords = set(
                page_name.lower().replace('-', ' ').replace('_', ' ').split()
            )
            page_topic_keywords -= {'the', 'a', 'an', 'and', 'or', 'for', 'in', 'of', 'to'}

        doc_keywords = set(
            doc_name.replace('.md', '').replace('.rst', '').replace('.txt', '')
    ...
    def _get_doc_nodes_fts5(
        self, fts, target_docs, page_name, target_symbols,
        page_keywords, symbol_keywords, max_docs,
    ):
        ...
        if target_docs:
            ...
            for target in target_docs:
                rows = fts.search_by_path_prefix(
                    target, symbol_types=_DOC_TYPES, k=5,
                )
                for row in rows:
                    ...
                    score = self._score_doc_candidate(
                        fp, content, target_docs, page_keywords, symbol_keywords, page_name,
                    )
                    doc_candidates.append((score, fp, row, content, row.get('symbol_type', '')))
            logger.info(
                f"[DOC_RETRIEVAL][FTS5] target_docs path search → {len(doc_candidates)} candidates"
            )
        else:
            ...
            logger.info(
                f"[DOC_RETRIEVAL][FTS5] semantic query={fts_query!r:.60} → "
                f"{len(doc_candidates)} candidates"
            )
        return doc_candidates

    def _get_doc_nodes_brute(
        self, graph, target_docs, page_name, target_symbols,
        page_keywords, symbol_keywords, max_docs,
    ):
        ...
        for node_id, node_data in graph.nodes(data=True):
            ...
            if target_docs:
                path_match = any(
                    t in file_path or file_path.endswith(t)
            ...
            score = self._score_doc_candidate(
                file_path, content, target_docs, page_keywords, symbol_keywords, page_name,
            )
            doc_candidates.append((score, file_path, node_data, content, symbol_type))

        logger.info(
            f"[DOC_RETRIEVAL][BRUTE] O(N) scan → {len(doc_candidates)} candidates"
        )
        return doc_candidates
    ...
    def _fetch_explicit_target_docs(
        self, target_docs: List[str], token_cap: int = 0
    ) -> List[Document]:
        """
        ...
        if fts is not None and fts.is_open:
            ...
            for target in target_docs:
                if total_chars >= char_cap:
                    break
                rows = fts.search_by_path_prefix(
                    target, symbol_types=_DOC_TYPES, k=3,
                )
                for row in rows:
                    fp = row.get('rel_path') or row.get('file_path', '')
                    if fp in seen_paths:
        ...
        still_missed = [
            t for t in target_docs
            if not any(t in d.metadata.get('source', '') for d in collected)
        ]
        ...
        if collected:
            ...
            source_info = (
                f" ({disk_count} from disk)" if disk_count else ""
            )
            logger.info(
    ...
    def _get_relevant_content_for_page(self, page_spec: PageSpec, repo_url: str, repo_context: str) -> Dict[str, Any]:
        ...
        # ── Cluster-expansion fast path (Phase 6) ──────────────────────
        ...
        if has_symbols:
            logger.info(f"[CONTENT_RETRIEVAL] Using graph-first retrieval with {len(target_symbols)} target symbols: {target_symbols[:5]}...")
        ...
        if not has_symbols or not code_found:
            ...
            if not graph_docs:
                if SEPARATE_DOC_INDEX or USE_SEMANTIC_DOC_RETRIEVAL:
                    ...
                    if doc_nodes:
                        logger.info(f"[DOC_RETRIEVAL] Doc-focused page: adding {len(doc_nodes)} docs from semantic retrieval")
                    ...
                    if doc_nodes:
                        logger.info(f"[DOC_RETRIEVAL] Doc-focused page: adding {len(doc_nodes)} documentation nodes from graph")
                        graph_docs.extend(doc_nodes)
        ...
        if graph_docs:
            ...
            if graph_tokens <= CONTEXT_TOKEN_BUDGET:
                ...
                truncated_docs = self._ranked_truncation(graph_docs, page_spec, CONTEXT_TOKEN_BUDGET)
                return self._format_simple_context(truncated_docs, page_spec)
        else:
            if target_symbols:
                logger.warning(f"[GRAPH_RETRIEVAL] No documents found for target_symbols={target_symbols}, falling back to vector search")
            else:
                logger.info(f"[CONTENT_RETRIEVAL] No target_symbols for page '{page_spec.page_name}', using vector search fallback")
        ...
        try:
            ...
            logger.info(f"Total context tokens: {total_tokens:,} (budget: {CONTEXT_TOKEN_BUDGET:,}, "
            ...
            if total_tokens <= CONTEXT_TOKEN_BUDGET:
                ...
                truncated_docs = self._ranked_truncation(relevant_docs, page_spec, CONTEXT_TOKEN_BUDGET)
                return self._format_simple_context(truncated_docs, page_spec)
    ...
    def _create_repository_tree(self, file_paths: List[str], max_files_per_dir: int = 25) -> str:
        ...
        tree_lines = [
            f"Total files: {len(unique_paths)}",
    ...
    def _llm_analyze_repository(self, repository_tree: str, readme_content: str,
                               code_samples: str, file_paths: List[str], config: RunnableConfig) -> str:
        ...
        if USE_STRUCTURED_REPO_ANALYSIS:
            ...
            analysis_prompt = ChatPromptTemplate.from_messages([
                ("system", "You are an expert repository analyst. Output ONLY valid JSON matching the specified schema. No markdown, no explanations - just the JSON object."),
    ...
    def _extract_representative_code_samples_from_files(self, file_paths: List[str]) -> str:
        ...
        config_files = [
            ...
            if any(p.lower().endswith(ext) for ext in [
                '.yaml', '.yml', '.json', '.toml', '.ini',
                '.proto', '.tf', '.tfvars', '.gradle', '.kts', '.wsdl', '.xsd', '.hcl',
            ])
        ]
                ...
                try:
                    for _, target, edge_data in graph.out_edges(node_id, data=True):
                        ...
                        if edge_type in ('imports', 'file_imports'):
                            ...
                            target_name = (
                                target_data.get('symbol_name') or
                                target_data.get('name') or
                                target
                            )
    ...
    def _ranked_truncation(self, docs: List[Any], page_spec: PageSpec, budget: int) -> List[Any]:
        """
        ...
        Returns:
            List of documents that fit within budget (full content preserved)
        """
        from ..document_ranker import DocumentRanker
        ...
        ranker = DocumentRanker(
            code_graph=self.retriever_stack.relationship_graph if hasattr(self.retriever_stack, 'relationship_graph') else None
        )
        page_spec_dict = {
        ...
        tokens_used = 0
        dropped = 0
        for doc, tier, score in sorted_docs:
    ...
    def _format_simple_context(self, relevant_docs: List[Any], page_spec: PageSpec) -> Dict[str, Any]:
        ...
        logger.info(f"Simple mode: {len(relevant_docs)} docs, {len(documentation_docs)} doc files, {len(docs_by_file)} code files")
    ...
    def _format_hierarchical_context(self, relevant_docs: List[Any], page_spec: PageSpec) -> Dict[str, Any]:
        ...
        graph = self.retriever_stack.content_expander.graph if hasattr(self.retriever_stack, 'content_expander') else None
        ...
        logger.info(f"Hierarchical mode: {len(relevant_docs)} total docs → " +
    ...
    def _generate_simple(self, page_spec: PageSpec, relevant_content: Dict[str, Any], 
        ...
        content_prompt = ChatPromptTemplate.from_messages([
            ("system", "You are an expert technical writer creating comprehensive, enterprise-grade documentation."),
    ...
    def _generate_agentic(self, page_spec: PageSpec, relevant_content: Dict[str, Any],
                          repo_context: str, config: RunnableConfig) -> str:
        """
        ...
        generator = AgenticDocGeneratorV2(
            ...
            wiki_style=self.wiki_style.value,
            target_audience=target_audience,
            graph=self.retriever_stack.relationship_graph if hasattr(self.retriever_stack, 'relationship_graph') else None,
            retriever_stack=self.retriever_stack
        )

        if os.getenv("DEEPWIKI_AGENTIC_DROP_EXPANDED_DOCS", "0") == "1":
    ...
    def _generate_with_continuation(self, page_spec: PageSpec, relevant_content: Dict[str, Any],
        ...
        effective_repo_context = "" if SKIP_REPO_CONTEXT_FOR_PAGES else repo_context
        if SKIP_REPO_CONTEXT_FOR_PAGES:
            logger.info(f"⚡ SKIP_REPO_CONTEXT_FOR_PAGES=1: Omitting {len(repo_context):,} char repository_context (hierarchical)")

        # Build initial message history
        messages = [
            SystemMessage(content="You are an expert technical writer creating comprehensive, enterprise-grade documentation."),
            HumanMessage(content=ENHANCED_CONTENT_GENERATION_PROMPT_V3_WITH_CONTINUATION.format(
                section_name=page_spec.page_name.split('/')[0] if '/' in page_spec.page_name else "Main",
                page_name=page_spec.page_name,
                page_description=page_spec.description,
                content_focus=page_spec.content_focus,
        ...
        for iteration in range(max_iterations):
            ...
            logger.info(
                f"🔍 LLM requested {len(need_context_requests)} continuation(s) for '{page_spec.page_name}'"
            )
            ...
            if not fetched_docs:
                ...
                logger.warning(
                    f"⚠️ Could not fetch any requested items for '{page_spec.page_name}', "
                    f"returning current content"
                )
                accumulated_content += generated_content
                return self._strip_need_context_markers(accumulated_content)
...
Please continue writing the documentation from where you stopped, incorporating this additional information. Maintain the same tone and structure.{iteration_status}"""
            ...
            logger.info(f"📝 Continuing generation with {len(fetched_docs)} additional documents (iteration {current_iteration}/{max_iterations})")
    ...
    def _fetch_requested_items(self, items: List[str], all_docs: List[Any], 
                              token_limit: int = 40000) -> tuple[List[Any], Dict[str, Any]]:
    ...
    def _find_node_in_graph(self, symbol_name: str, file_path: str, graph) -> Optional[str]:
        ...
        for node_id, node_data in graph.nodes(data=True):
            if (node_data.get('symbol_name') == symbol_name and 
